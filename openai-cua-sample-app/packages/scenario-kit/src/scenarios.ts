...
export function buildCustomScenarioManifest(prompt: string): ScenarioManifest {
  return scenarioManifestSchema.parse({
    id: CUSTOM_SCENARIO_ID,
    labId: "custom",
    category: "custom",
    title: "Custom Task",
    description: prompt.slice(0, 200) || "Custom browser task",
    defaultPrompt: prompt,
    workspaceTemplatePath: "__custom__",
    startTarget: {
      kind: "remote_url",
    ...
    verification: [
      {
        id: "custom-none",
        kind: "none",
        description: "No verification for custom runs.",
      },
    ],
    tags: ["custom"],
  });
