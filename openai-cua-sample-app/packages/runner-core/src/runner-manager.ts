...
import {
  buildCustomScenarioManifest,
  CUSTOM_SCENARIO_ID,
  getScenarioById,
} from "@cua-sample/scenario-kit";
...
export class RunnerManager {
  ...
  async startRun(input: StartRunRequest): Promise<RunDetail> {
    ...
    const isCustom = !request.scenarioId || request.scenarioId === CUSTOM_SCENARIO_ID;
    const scenario = isCustom
      ? buildCustomScenarioManifest(request.prompt)
      : getScenarioById(request.scenarioId!);
    ...
    const runRecord = runRecordSchema.parse({
      ...
      startedAt,
      statelessMode: request.statelessMode ?? false,
      status: "running",
  ...
  private async readRunDetail(runId: string): Promise<RunDetail> {
    try {
      ...
      const scenario = run.scenarioId === CUSTOM_SCENARIO_ID
        ? buildCustomScenarioManifest(run.prompt)
        : getScenarioById(run.scenarioId);
