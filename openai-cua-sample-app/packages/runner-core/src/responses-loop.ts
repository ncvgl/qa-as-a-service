...
function summarizeActionForLog(action: ComputerAction): string {
  switch (action.type) {
    case "click":
    ...
    case "keypress":
      return `keypress(${action.keys ?? action.key})`;
    case "scroll":
      return `scroll(dx=${action.delta_x ?? action.deltaX ?? 0}, dy=${action.delta_y ?? action.deltaY ?? action.scroll_y ?? 0})`;
    case "drag":
      return `drag(${Array.isArray(action.path) ? action.path.length : 0} points)`;
    case "move":
      return `move(${action.x}, ${action.y})`;
    case "wait":
      return `wait(${action.ms ?? action.duration_ms ?? 1000}ms)`;
    case "screenshot":
      return "screenshot";
    default:
      return action.type;
...
export async function runResponsesNativeStatelessLoop(
  input: ResponsesLoopContext,
  client: ResponsesClient,
): Promise<ResponsesLoopResult> {
  const operatorPrompt = input.prompt ?? input.context.detail.run.prompt;
  ...
  for (let turn = 1; turn <= input.maxResponseTurns; turn += 1) {
    ...
    const pageTitle = await input.session.page.title();

    const contextSummary = turnLog.length > 0
    ...
    const freshInput = [
      {
        content: [
          {
            text: `${operatorPrompt}${contextSummary}`,
            type: "input_text",
          },
          {
    ...
    for (const outputItem of response.output ?? []) {
      if (isFunctionCallItem(outputItem)) {
        ...
        await executeFunctionToolCall(input, outputItem);
        turnActions.push(`function_call: ${outputItem.name}`);
        continue;
      }
      ...
      for (const action of actions) {
        await executeComputerAction(input, action);
        turnActions.push(summarizeActionForLog(action));
      }
      ...
      await input.context.captureScreenshot(
        input.session,
        `responses-stateless-turn-${turn}`,
      );
    }

    const modelText = extractAssistantMessageText(response);
    const actionSummary = turnActions.join(", ");
    turnLog.push(`Turn ${turn}: ${actionSummary}${modelText ? ` — ${modelText}` : ""}`);
  }

  if (!finalAssistantMessage) {
    throw new Error(
      `Responses API stateless native loop exhausted the configured ${input.maxResponseTurns}-turn budget without producing a final assistant message.`,
    );
  }
  ...
  return {
    finalAssistantMessage,
    notes: [
      "Executed the scenario through a stateless Responses API native loop (1 screenshot per turn).",
      `Model final response: ${finalAssistantMessage}`,
    ],
  };
}
