...
export function useRunStream({
  ...
  const isCustom = selectedScenarioId === "__custom__" || customPresets.some((p) => p.id === selectedScenarioId);
  const selectedScenario =
    scenarios.find((scenario) => scenario.id === selectedScenarioId) ??
    (isCustom ? null : initialScenario);
  const runnerOnline = !initialRunnerIssue && scenarios.length > 0;
  const selectedRun =
    activeRun &&
    (isCustom
      ? activeRun.run.scenarioId === "__custom__"
      : selectedScenario && activeRun.run.scenarioId === selectedScenario.id)
      ? activeRun
      : null;
  ...
  const handleScenarioChange = (scenarioId: string) => {
    ...
    const preset = customPresets.find((p) => p.id === scenarioId);

    if (scenarioId === "__custom__" || preset) {
      if (!selectedRun || selectedRun.run.status !== "running") {
        setActiveRun(null);
        setRunEvents([]);
      }
      setMode(preset?.mode ?? "native");
      setPrompt(preset?.prompt ?? "");
      if (preset?.stateless !== undefined) {
        setStatelessMode(preset.stateless);
      }
      return;
    }
  ...
  const handleStartRun = async () => {
    if (!runnerOnline || (!isCustom && !selectedScenario) || prompt.trim().length === 0) {
      return;
    }
    ...
    const runTitle = isCustom ? "Custom Task" : selectedScenario!.title;
    ...
    try {
      const started = await requestJson(
        ...
        startRunResponseSchema,
        {
          body: JSON.stringify({
            ...
            prompt,
            ...(isCustom ? {} : { scenarioId: selectedScenario!.id }),
            statelessMode,
            verificationEnabled: isCustom ? false : verificationEnabled,
          }),
          headers: {
      ...
      appendManualTranscript(
        createManualTranscript(
          "control",
          "operator",
          `Run ${started.runId} started for ${runTitle}.`,
        ),
      );
    } catch (error) {
  ...
  const handleResetWorkspace = async () => {
    if (!runnerOnline || !selectedScenario || isCustom) {
      return;
    }
