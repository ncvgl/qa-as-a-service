...
ch = greedy(base, 200)
positions = [0, 4, 8, 12, 16, 20, 24, 32, 48]
print("LONG-WINDOW (N=180) recovery — does the thesis sentence commit invasions?", flush=True)
res = []
for p in positions:
    samp = bgen([bl + ch[:p]] * M, N_LONG)
    rs = float(np.mean([hit(g) for g in samp]))
    res.append({"pos": p, "rec_long": rs, "ctx": tok.decode(ch[max(0,p-6):p], skip_special_tokens=True)})
    print(f" p={p:>3} rec_long={rs:4.2f}  ...{res[-1]['ctx'][-30:]!r}  ({time.time()-t0:.0f}s)", flush=True)
json.dump(res, open("/tmp/curve_4b_confirm.json", "w"), indent=2)
print("WROTE /tmp/curve_4b_confirm.json")
